...
// Direct client fallback if running in purely static environment
async function scanJobsDirectly(employerName: string, website: string, existingTitles?: string[]): Promise<ScannedJob[]> {
  const ai = getClientAI();
  const existingTitlesArray = Array.isArray(existingTitles) ? existingTitles : [];
  
  if (!ai) {
    return generateDomainFallbackJobs(employerName, website, existingTitlesArray);
  }
...
If no jobs exist, return an empty array [].`;
  ...
  // Attempt 1: Direct High-Speed Model Generation with backoff
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const status = err?.status || (String(err?.message).includes("503") ? 503 : (String(err?.message).includes("429") ? 429 : 500));
      if (attempt < maxRetries - 1 && (status === 503 || status === 429 || status === 500)) {
        const delay = Math.pow(2, attempt) * 500 + Math.floor(Math.random() * 200);
        console.warn(`[Client Retry] ${primaryModel} returned status ${status} for ${employerName}. Retrying in ${delay}ms...`);
        await new Promise(r => setTimeout(r, delay));
      }
    }
  }

  // Attempt 2: Optional Search Grounding Fallback
  try {
